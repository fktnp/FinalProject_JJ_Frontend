...
class ToDoListState extends State<ToDoList> {
  ...
  Future<void> _completeTask(String taskId) async {
    try {
      final response = await _dio.get('http://10.0.2.2:8080/v1/calendar/task/$taskId'); // เปลี่ยน URL ตามที่คุณใช้
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          if (taskIndex != -1) {
            tasks[taskIndex].isCompleted = true; // เปลี่ยนสถานะให้เป็น completed
          }
        });
      } else {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          final taskIndex = tasks.indexWhere((task) => task.id == taskId);
          if (taskIndex != -1) {
            tasks[taskIndex].isCompleted = false; // เปลี่ยนสถานะให้เป็น not completed
          }
        });
      } else {
          ...
          child: Column(
            children: [
              ...
              ShowListTask(
                ...
                onTaskCompleted: _completeTask, // ส่ง callback สำหรับ task ที่เสร็จแล้ว
                onTaskUncompleted: _uncompleteTask, // ส่ง callback สำหรับ task ที่ไม่สำเร็จ
...
class ShowListTask extends StatelessWidget {
  ...
  final Function(String) onTaskUncompleted; // ฟังก์ชันสำหรับทำให้ Task ไม่สำเร็จ
              ...
              child: Container(
                ...
                child: Dismissible(
                  ...
                  confirmDismiss: (direction) async {
                    if (direction == DismissDirection.startToEnd) { //อันนี้ปัดติ๊กถูก
                      onTaskCompleted(task.id); // เรียกการทำงานเมื่อมีการติ๊กถูก
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(  //ไว้ทำการแจ้งเตือนการปัด
                          content: Text('${task.title} marked as completed'),
                          duration: const Duration(seconds: 2),
                        ),
                      );
                      return false;
                    } else if (direction == DismissDirection.endToStart) { //อันนี้ปัดยกเลิก
                      onTaskUncompleted(task.id); // เรียกการทำงานเมื่อมีการยกเลิกติ๊กถูก
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar( //ไว้ทำการแจ้งเตือนการปัด
