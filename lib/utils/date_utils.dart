...
class DateUtils {
  ...
  static List<Map<String, String>> getWeekDays(DateTime currentDate) {
  ...
  static List<String> getMonths() {
    final formatter = DateFormat('MMM'); // ภาษาอังกฤษ
    final months = <String>[];

    for (int i = 1; i <= 12; i++) {
      final date = DateTime(2020, i, 1); // ใช้ปี 2020 เพื่อให้วันที่ถูกต้อง
  ...
  static List<String> getYears(DateTime currentDate) {
    final years = <String>[];

    for (int i = -10; i <= 10; i++) { // เพิ่มช่วงให้กว้างขึ้น
      final date = DateTime(currentDate.year + i, 1, 1);
