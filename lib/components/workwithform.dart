  ...
  Future<void> _updateParticipantsInTeamJob(String userId) async {
    ...
    try {
      ...
      final response = await Dio().put(
        'http://192.168.1.36:8080/v1/teamJob/${widget.teamjobmodel.jobId}',
        data: data,
      );
...
class _AddTeamSubParticipantPopupState
  ...
  Future<void> _updateParticipantsInTeamSubJob(String userId) async {
    ...
    try {
      // ใช้ `put` แทน `post` เพื่ออัปเดตข้อมูล
      final response = await Dio().put(
        'http://192.168.1.36:8080/v1/teamSubJob/${widget.teamsubJobmodel.jobId}',
        data: data,
      );
...
class AddTeamSubWorkAreaState extends State<AddTeamSubWorkArea> {
  ...
  Future<void> _updateParticipantsInTeamSubJob() async {
    ...
    try {
      // ใช้ `put` แทน `post` เพื่ออัปเดตข้อมูล
      final response = await Dio().put(
        'http://192.168.1.36:8080/v1/teamSubJob/${widget.teamsubJobmodel.subJobId}',
        data: data,
      );
      ...
      if (response.statusCode == 200) {
        print('Update successful');
        print(
            'With http://192.168.1.36:8080/v1/teamSubJob/${widget.teamsubJobmodel.subJobId} By $data');
      } else {
        print('Update failed with status: ${response.statusCode}');
      }
    } on DioException catch (e) {
